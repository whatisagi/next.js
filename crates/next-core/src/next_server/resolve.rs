...
impl AfterResolvePlugin for ExternalCjsModulesResolvePlugin {
    ...
    async fn after_resolve(
        ...
        let typ = match (file_type, is_esm) {
            ...
            (FileType::CommonJs, false) => (name, ExternalType::CommonJs),
            (FileType::CommonJs, true) => {
                ...
                match (must_be_external, resolves_equal) {
                    ...
                    (_, true) => (name, ExternalType::CommonJs),
                    (_, false) => (name, ExternalType::EcmaScriptModule),
                }
            }
            // ecmascript with esm is always external
            (FileType::EcmaScriptModule, true) => (name, ExternalType::EcmaScriptModule),
        ...
        Ok(ResolveResultOption::some(
            ResolveResult::primary(ResolveResultItem::External {
                name,
                typ,
                source: Some(Vc::upcast(FileSource::new(path))),
            })
            .cell(),
        ))
    }
